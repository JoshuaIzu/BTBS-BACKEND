...
const registerUser = async (req, res) => {
    ...
    try {
        ...
        const validRoles = ["commuter", "business"];
        ...
        res.status(201).json({ message: 'User registered successfully. OTP email sent.', user: userResponse });
    } catch (error) {
...
const sendLoginOtp = async (req, res) => {
    ...
    try {
        ...
        if (!user.isVerified) {
            return res.status(403).json({ message: 'Please verify your email before requesting a login OTP' });
        }
